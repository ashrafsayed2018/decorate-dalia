        ...
        <ul className="flex items-center gap-8 pt-[15px] mt-4 ">
          {SocialShareLinks.map((link) => {
            return (
              <li
                key={link.id}
                className="cursor-pointer w-[90px] h-[90px] rounded-full flex items-center justify-center border-[2px] border-white hover:border-blue-600 transition-all duration-500 group"
              >
                <Link href={link.url}>
                  <FontAwesomeIcon
                    icon={link.icon}
